...
export ETCD_INITIAL_CLUSTER=\${ETCD_INITIAL_CLUSTER:-'{{range \$i, \$containerName := ls "/service/containers"}}{{if \$i}},{{end}}{{getv (printf "/service/containers/%s/name" \$containerName)}}={{getv (printf "/service/containers/%s/primary_ip" \$containerName)}}:2380{{end}}'}
export ETCD_INITIAL_CLUSTER_STATE=\${ETCD_INITIAL_CLUSTER_STATE:-"new"}
export ETCD_INITIAL_CLUSTER_TOKEN=\${ETCD_INITIAL_CLUSTER_TOKEN:-'{{getv "/self/container/service_name"}}-{{getv "/self/container/stack_name"}}'}
export ETCD_NAME=\${ETCD_NAME:-'{{\$my_name}}'}
export ETCD_LISTEN_PEER_URLS=\${ETCD_LISTEN_PEER_URLS:-'http://{{\$my_ip}}:2380'}
export ETCD_LISTEN_CLIENT_URLS=\${ETCD_LISTEN_CLIENT_URLS:-'http://{{\$my_ip}}:2379'}
EOF
